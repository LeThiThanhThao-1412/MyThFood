...
*(Xử lý aggregate ở frontend: nhóm theo ngày, tính tổng)*
...
*(Frontend aggregate: nhóm theo ngày/tuần/tháng, tính tổng)*
...
*(Cần thêm admin analytics endpoints - xem API Gaps)*
...
| Trang | API | UI |
|-------|-----|-----|
| `/users` | *(Cần admin users endpoint)* | Table: ID, Tên, Email, Role, Trạng thái, Actions |
...
| `/transactions` | *(Cần admin transactions endpoint)* | Table filter |
