...
function formatVndCompact(amount: number): string {
  if (amount >= 1_000_000_000)
    return `${(amount / 1_000_000_000).toFixed(1)}tỷđ`;
...
export default function AdminDashboardPage() {
  ...
  const [tab, setTab] = useState<"overview" | "merchants" | "drivers">(
    "overview",
  );
  ...
  const [recentActivity, setRecentActivity] = useState<
    { icon: string; text: React.ReactNode; time: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [mSettled, dSettled, oSettled] = await Promise.allSettled([
          merchantApi.list({ take: 500 }),
          driverApi.list({}),
          orderApi.list({ take: 1000 }),
        ]);
        const mRes: any =
          mSettled.status === "fulfilled" ? mSettled.value : { items: [] };
        const dRes: any =
          dSettled.status === "fulfilled" ? dSettled.value : { data: [] };
        const oRes: any =
          oSettled.status === "fulfilled" ? oSettled.value : { items: [] };
        ...
        for (const o of orders) {
          ...
          if (mid) {
            if (!merchantRevenue[mid]) {
              merchantRevenue[mid] = {
                name: mMap.get(mid) || mid.slice(0, 8),
                orders: 0,
                revenue: 0,
              };
            }
        ...
        // Fetch platform wallet (income from commissions)
        try {
          const pw = await walletApi.getWallet("PLATFORM_DEFAULT", "PLATFORM");
          setPlatformWallet({
            balance: Number(pw.balance) || 0,
            transactions: pw.transactions || [],
          });
          setPlatformRevenue(Number(pw.balance) || 0);
        } catch {
          /* wallet service may be down */
        }
            ...
            : [
                {
                  icon: "📋",
                  text: (
                    <span className="text-gray-400">Chưa có hoạt động nào</span>
                  ),
                  time: "",
                },
              ],
  ...
  async function handleActivateDriver(id: string) {
    ...
    try {
      ...
      setDrivers(
        drivers.map((d) => (d.id === id ? { ...d, status: "ACTIVE" } : d)),
      );
      setDStats((s) => ({
        ...s,
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5]">
      ...
      {/* Sub-page navigation */}
      <div className="max-w-[1400px] xl:mx-auto px-4 pt-4 flex gap-2 overflow-x-auto">
        <Link
          href="/"
          className="shrink-0 px-4 py-2 rounded-xl text-sm font-semibold bg-[#ff6b35] text-white shadow-sm"
        >
          📊 Tổng quan
        </Link>
        <Link
        ...
        <div className="flex gap-2 mb-6 bg-white rounded-2xl p-1.5 shadow-sm w-fit">
          {(
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === t.key ? "bg-[#ff6b35] text-white shadow-md shadow-orange-200" : "text-gray-500 hover:text-gray-800 hover:bg-gray-50"}`}
            >
              {t.icon} {t.label}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <>
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  {[
                    ...
                    },
                    {
                      ...
                      sub: `${mStats.approved} đang hoạt động`,
                      up: true,
                    },
                    {
                      icon: "📦",
                    ...
                    >
                      ...
                      <p className="text-[28px] font-extrabold mt-1 text-[#1a1a2e]">
                        {s.value}
                      </p>
                      <p
                        className={`text-xs mt-1 font-medium ${s.up ? "text-[#2ecc71]" : "text-gray-400"}`}
                      >
                        {s.sub}
                      </p>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold">💰 Ví nền tảng</h3>
                    <span className="text-xs text-gray-400">
                      Thu nhập từ phí chiết khấu
                    </span>
                  </div>
                  <div className="bg-gradient-to-r from-[#8b5cf6] to-[#6d28d9] rounded-2xl p-5 text-white mb-4">
                    <p className="text-sm text-white/80">Số dư ví nền tảng</p>
                    <p className="text-3xl font-extrabold mt-1">
                      {formatVnd(platformWallet.balance)}
                    </p>
                  </div>
                  <p className="text-sm font-semibold text-gray-700 mb-2">
                    📋 Lịch sử thu nhập gần đây
                  </p>
                  {platformWallet.transactions.length === 0 ? (
                    ...
                    {maxWeekly === 0 ? (
                      ...
                      <div className="flex items-end gap-2 h-[170px] pt-2">
                        {weeklyRevenue.map((val, i) => (
                          ...
                          >
                            <span className="text-[10px] font-semibold text-[#1a1a2e] mb-1">
                              {val > 0 ? formatVnd(val) : ""}
                            </span>
                            <div
                              className={`w-full bg-gradient-to-t ${GRADIENT_BARS[i]} rounded-t-md transition-all duration-500`}
                              style={{
                                height: `${Math.max(4, (val / maxWeekly) * 130)}px`,
                              }}
                            />
                        ...
                        topMerchants.map((m, i) => (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <span className="font-bold text-sm text-gray-400 w-6">
                                #{i + 1}
                              </span>
                            ...
                            <span className="font-bold text-sm text-[#ff6b35] whitespace-nowrap ml-2">
                              {formatVnd(m.revenue)}
                            </span>
                          </div>
                        ))
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  {[
                    ...
                    {
                      label: "Chờ duyệt",
                      value: mStats.pending,
                      color: "bg-[#e67e22]",
                    },
                    {
                      label: "Đã duyệt",
                      value: mStats.approved,
                      color: "bg-[#2ecc71]",
                    },
                    {
                      label: "Từ chối",
                      value: mStats.rejected,
                      color: "bg-[#e74c3c]",
                    },
                  ].map((s, i) => (
                    <div
                      key={i}
                      className={`${s.color} text-white rounded-2xl p-5 shadow-sm`}
                    >
                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                  <table className="w-full">
                    <thead className="bg-gray-50/80 border-b border-gray-100">
                      <tr>
                        <th className="text-left px-4 sm:px-6 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                          Tên nhà hàng
                        </th>
                        <th className="text-left px-4 sm:px-6 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider hidden md:table-cell">
                          Địa chỉ
                        </th>
                        <th className="text-left px-4 sm:px-6 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider hidden lg:table-cell">
                          SĐT
                        </th>
                        <th className="text-left px-4 sm:px-6 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                          Trạng thái
                        </th>
                        <th className="text-right px-4 sm:px-6 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                          Hành động
                        </th>
                      </tr>
                    </thead>
                      ...
                      {merchants.length === 0 ? (
                        ...
                        merchants.map((m: any) => (
                          ...
                          >
                            ...
                            <td className="px-4 sm:px-6 py-4 text-sm text-gray-500 hidden md:table-cell max-w-[200px] truncate">
                              {m.address}
                            </td>
                            ...
                            <td className="px-4 sm:px-6 py-4">
                              <span
                                className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${statusBadge(m.status, "merchant").cls}`}
                              >
                                {statusBadge(m.status, "merchant").label}
                              </span>
                            </td>
                              ...
                              {m.status === "PENDING" ? (
                                <div className="flex gap-2 justify-end">
                                  ...
                                  <button
                                    ...
                                    className="bg-[#e74c3c] text-white text-xs px-3 py-1.5 rounded-lg hover:bg-red-600 transition font-medium"
                                  >
                                    Từ chối
                                  </button>
                                </div>
            ...
            {tab === "drivers" && (
              <>
                {actionError && (
                  <div className="bg-red-50 border border-red-200 text-red-600 p-4 rounded-xl text-sm mb-4 flex items-start gap-2">
                    <span>⚠️</span>
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {drivers.map((d: any) => (
                          ...
                          >
                            ...
                            <td className="px-4 sm:px-6 py-4">
                              <span
                                className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${statusBadge(d.status, "driver").cls}`}
                              >
                                {statusBadge(d.status, "driver").label}
                              </span>
                            </td>
                            <td className="px-4 sm:px-6 py-4 text-right">
                              <div className="flex gap-2 justify-end">
