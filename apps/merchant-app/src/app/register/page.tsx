  ...
  async function createMerchant(e: React.FormEvent) {
    ...
    try {
      await merchantApi.create({
        ...
        secondaryCategories: merchant.secondaryCategories,
      });
      setStep("done");
    } catch (err: unknown) {
  ...
  function toggleSecondaryCategory(key: string) {
    setMerchant((prev) => {
      ...
      const next = current.includes(key)
        ...
        : [...current, key];
      return { ...prev, secondaryCategories: next };
    });
  }
            ...
            <form onSubmit={createAccount} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border px-4 py-3 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none"
                  required
                />
              </div>
              <div>
            ...
            <form onSubmit={createMerchant} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-2">
                  Tìm địa chỉ hoặc kéo thả điểm trên bản đồ để chọn vị trí chính
                  xác
                </p>
                  ...
                  {MERCHANT_CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border-2 text-xs font-medium transition ${
                        merchant.primaryCategory === cat.key
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Thêm loại phụ (tùy chọn)
                </label>
                <div className="flex flex-wrap gap-2">
                  {MERCHANT_CATEGORIES.filter(
                    (c) => c.key !== merchant.primaryCategory,
                  ).map((cat) => {
                    const active = merchant.secondaryCategories.includes(
                      cat.key,
                    );
                    return (
                      <button
        ...
        {step === "done" && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 mb-4">
              Nhà hàng của bạn đang chờ Admin duyệt.
            </p>
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 text-sm text-yellow-800">
              <p className="font-medium mb-1">⏳ Trạng thái: PENDING</p>
              <p>
                Admin sẽ duyệt nhà hàng của bạn trong vòng 24-48h. Bạn có thể
                đăng nhập để theo dõi trạng thái.
              </p>
            </div>
            <button
