  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-100 animate-fade-in-up">
      ...
      <div className="mb-3">
        ...
        <div
          className={`relative rounded-xl border bg-gray-50 px-4 py-3 transition-all ${
            cardError
              ? "border-red-300 bg-red-50"
              : "border-gray-200 focus-within:border-purple-400 focus-within:bg-white focus-within:ring-2 focus-within:ring-purple-200"
          }`}
        >
          <CardNumberElement
            options={{
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          ...
          <div
            className={`rounded-xl border bg-gray-50 px-4 py-3 transition-all ${
              expiryError
                ? "border-red-300 bg-red-50"
                : "border-gray-200 focus-within:border-purple-400 focus-within:bg-white focus-within:ring-2 focus-within:ring-purple-200"
            }`}
      ...
      <div className="flex items-center gap-2 justify-center">
        ...
        <span className="text-[10px] bg-green-50 text-green-600 px-2 py-0.5 rounded font-semibold border border-green-200">
          🔒 3DS
        </span>
      </div>
    </div>
...
function CheckoutContent() {
  ...
  const { items, merchantId, merchantName, clearCart, getSubtotal } =
    useCartStore();
  ...
  const applyPromo = async (promo?: Promotion | null) => {
    if (!promo || !promo.code) return;
    const code = promo.code.trim().toUpperCase();
    if (!merchantId) {
      setError("Thiếu thông tin nhà hàng");
      return;
    }
    setError("");
    try {
      const itemTotal =
        promo.target === "ITEM"
          ? items
              .filter((i) => i.menuItem?.id === promo.menuItemId)
              .reduce(
                (s, i) =>
                  s + (i.unitPrice ?? i.menuItem?.price ?? 0) * i.quantity,
                0,
              )
          : 0;
      const res: any = await promotionApi.validate({
        merchantId,
        code,
        foodTotal: subtotal,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const p: any = await promotionApi.getByMerchant(merchantId);
        ...
        setAvailablePromos(
          list.filter((x: any) => {
            ...
            if (x.startAt && new Date(x.startAt).getTime() > now) return false;
            if (x.endAt && new Date(x.endAt).getTime() < now) return false;
            return true;
          }),
        );
      } catch {
  ...
  const [merchantLat, setMerchantLat] = useState(10.77);
  const [merchantLng, setMerchantLng] = useState(106.7);
  // COD support from merchant settings
  const [merchantCodAccepted, setMerchantCodAccepted] = useState(true);
  // Merchant open status (manual toggle + operating hours)
    ...
    async function fetchMerchantInfo() {
      try {
        ...
        if (!cancelled && m) {
          ...
          // Track open status (manual close OR outside operating hours)
  ...
  function getSelectedAddress(): Address | undefined {
  ...
  function handleAddressSelect(addr: Address) {
    ...
    if (addr.latitude) setLat(addr.latitude);
    if (addr.longitude) setLng(addr.longitude);
    setShowSuggestions(false);
  }
  ...
  function selectSuggestion(s: Suggestion) {
    setCustomAddress(s.display_name);
  ...
  async function handlePlaceOrder() {
    ...
    // Re-check merchant is open (manual close or outside operating hours)
    try {
      const m = await merchantApi.getById(merchantId);
      if (m && (m.isOpen === false || m.isOpenNow === false)) {
        setError(
          m.isOpen === false
            ? "Nhà hàng đang tạm đóng cửa. Vui lòng thử lại sau."
            : "Nhà hàng đang ngoài giờ hoạt động. Vui lòng thử lại sau.",
        );
        return;
      }
    } catch {
      /* if we cannot verify, proceed */
    }
    ...
    try {
      // Proactively refresh token before payment to avoid mid-payment auth failure
      ...
      const orderBody: PlaceOrderRequest = {
        ...
        items: items.map((i) => ({
          ...
          options: i.selectedGroups?.flatMap((g) => g.options) ?? undefined,
        })),
      ...
      // ─── Stripe ──────────────────────────────────
        ...
        const { error: stripeErr, paymentIntent } =
          await stripe.confirmCardPayment(sp.clientSecret, {
        ...
        if (
          ...
          try {
            await paymentApi.complete(sp.id, {
            ...
            <div className="bg-[#fff7ed] rounded-2xl p-5 mb-6 text-left text-sm">
              <p className="font-bold text-gray-800 mb-2">
                ...
                <span className="text-[#ff6b35]">
                  #{successOrder.id.slice(0, 8)}
                </span>
              </p>
              <div className="space-y-1.5 text-gray-600">
                <p>🏪 {merchantName}</p>
                <p>📍 {successOrder.deliveryAddress}</p>
                <p className="font-bold text-[#ff6b35] text-base mt-2">
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5]">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-600 p-4 rounded-2xl text-sm flex items-start gap-3">
              ...
              {profileLoading ? (
                ...
                <>
                  <button
                    ...
                    className="w-full flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/50 hover:bg-blue-50 transition mb-4 text-left"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-blue-500">Tự động qua GPS</p>
                    </div>
                    {fetchingLocation && (
                    ...
                    <div className="space-y-2 mb-4">
                      {addresses.map((addr) => {
                        const t =
                          ...
                          ADDRESS_TYPE_LABELS.OTHER;
                        return (
                          <label
                            key={addr.id}
                            className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all ${selectedAddressId === addr.id ? "border-[#ff6b35] bg-[#fff7ed] ring-2 ring-orange-200" : "border-gray-100 hover:border-gray-300"}`}
                          >
                            <input
                              type="radio"
                              name="address"
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                ...
                                {addr.isDefault && (
                                  <span className="text-[10px] bg-orange-100 text-[#ff6b35] px-2 py-0.5 rounded-full font-semibold">
                                    Mặc định
                                  </span>
                                )}
                              </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!useCustomAddress) {
                        ...
                        const da = addresses.find((a) => a.isDefault);
                        if (da) handleAddressSelect(da);
                      }
                    }}
                    className="text-sm text-[#ff6b35] hover:text-orange-600 font-semibold mb-3 inline-block"
                  >
                    {useCustomAddress
                      ? "← Chọn địa chỉ đã lưu"
                      : "+ Nhập địa chỉ mới"}
                  ...
                  <div className="relative mb-3" ref={suggestionRef}>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-gray-200 bg-gray-50 pl-11 pr-4 py-3 text-sm focus:bg-white focus:border-[#ff6b35] focus:ring-2 focus:ring-orange-200 outline-none transition"
                      />
                    </div>
                      ...
                      <div className="absolute z-50 w-full mt-1 bg-white border border-gray-100 rounded-2xl shadow-lg max-h-60 overflow-y-auto">
                        {suggestions.map((s, i) => (
                          <button
                            ...
                            className="w-full text-left px-4 py-3 text-sm hover:bg-orange-50 border-b border-gray-50 last:border-b-0 transition"
                          >
            ...
            <section className="bg-white rounded-2xl shadow-sm p-5 sm:p-6">
              ...
              <div className="space-y-2.5">
                {/* Credit Card */}
                <label
                  className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${isStripe ? "border-purple-400 bg-purple-50/50 ring-2 ring-purple-200" : "border-gray-100 hover:border-gray-300"}`}
                >
                  <input
                    type="radio"
                    name="payment"
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Visa, Mastercard, JCB - Bảo mật Stripe
                    </p>
                  </div>
                  <div className="hidden sm:flex items-center gap-1.5">
                    <span className="text-xs bg-white border px-2 py-1 rounded font-medium text-gray-500">
                      Visa
                    </span>
                    <span className="text-xs bg-white border px-2 py-1 rounded font-medium text-gray-500">
                      MC
                  ...
                  <div className="space-y-2">
                    {paymentMethods
                      ...
                      .map((pm) => (
                        <label
                          key={pm.id}
                          className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 cursor-pointer hover:border-gray-300 transition"
                        >
                          <input
                            type="radio"
                            name="payment_saved"
                          ...
                          <span className="text-sm text-gray-700">
                            💳 {pm.provider} (••••{pm.lastFourDigits})
                          </span>
                          {pm.isDefault && (
                            <span className="text-[10px] bg-orange-100 text-[#ff6b35] px-2 py-0.5 rounded-full font-semibold ml-auto">
                              Mặc định
                            </span>
                          )}
                        </label>
              ...
              {isStripe && (
                <StripeCardFields
                  ...
                  setExpiryComplete={setExpiryComplete}
              ...
              {stripeError && (
                <div className="mt-3 bg-red-50 border border-red-200 text-red-600 p-3 rounded-xl text-sm flex items-start gap-2">
            ...
            <section className="bg-white rounded-2xl shadow-sm p-5 sm:p-6">
              ...
              <button
                ...
                className="w-full flex items-center justify-between gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 hover:border-[#ff6b35] transition"
              >
                <span className="flex items-center gap-2 text-sm">
                  <span>🏷️</span>
                  {promoApplied && discount > 0 ? (
                    <span className="font-semibold text-gray-800">
                      {promoCode} · Giảm {discount.toLocaleString("vi-VN")}₫
                    </span>
                  ) : (
                    <span className="text-gray-500">Chọn mã khuyến mãi</span>
                  )}
                </span>
                <span className="text-[#ff6b35] font-semibold">›</span>
              </button>
              {promoApplied && discount > 0 && (
                <div className="mt-3 bg-green-50 border border-green-200 text-green-700 px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-between gap-2">
                  <span className="flex items-center gap-2">
                    <span>🎉</span>Đã áp dụng! Giảm{" "}
              ...
              {!promoApplied && availablePromos.length > 0 && (
                <p className="text-xs text-gray-400 mt-3">
                  💡 Có {availablePromos.length} mã khuyến mãi đang khả dụng
                </p>
              )}
            </section>
            ...
            <section className="bg-white rounded-2xl shadow-sm p-5 sm:p-6">
              ...
              <textarea
                ...
                className="w-full bg-gray-50 rounded-xl border border-gray-200 px-4 py-3 text-sm focus:bg-white focus:border-[#ff6b35] focus:ring-2 focus:ring-orange-200 outline-none resize-none transition"
                rows={2}
              />
            </section>
          </div>
          ...
          <div>
            <div className="sticky top-20 space-y-4">
                    ...
                    {items.map((i) => (
                      ...
                      >
                        <span className="text-gray-700 flex items-center gap-2">
                          <span className="inline-flex items-center justify-center w-5 h-5 bg-[#fff7ed] text-[#ff6b35] rounded-full text-xs font-bold">
                            {i.quantity}
                          </span>
                          {i.menuItem.name}
                        </span>
                        <span className="text-gray-600 font-medium">
                          {(
                            (i.unitPrice ?? i.menuItem.price) * i.quantity
              ...
              {merchantOpen === false && (
                <div className="bg-red-50 border border-red-200 text-red-600 rounded-2xl p-4 flex items-center gap-3">
                  <span className="text-2xl">🔴</span>
                  <div>
                    ...
                    <p className="text-sm">
                      Nhà hàng đang đóng cửa hoặc ngoài giờ hoạt động.
                    </p>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                disabled={loading || profileLoading || merchantOpen === false}
                className="w-full bg-gradient-to-r from-[#ff6b35] to-[#ff8f65] text-white py-4 rounded-2xl font-bold text-base hover:shadow-lg hover:shadow-orange-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
                  ...
                  <>
                    <span className="inline-block w-5 h-5 border-2 border-white/50 border-t-transparent rounded-full animate-spin" />
                    Đang tải...
                  </>
                ) : (
                  <>
                    <span>🛵</span>Đặt hàng • {total.toLocaleString("vi-VN")}₫
                  </>
                )}
              </button>
              <p className="text-center text-xs text-gray-400">
                Bằng cách đặt hàng, bạn đồng ý với điều khoản của MyThFood
              </p>
              {dynamicFee && (
                <div className="mt-2 p-3 bg-gray-50 rounded-xl text-xs text-gray-500 space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{dynamicFee.distanceKm} km</span>
                  </div>
                  <div className="flex justify-between">
                    <span>💰 Phí cơ bản</span>
                    <span>{dynamicFee.baseFee.toLocaleString("vi-VN")}₫</span>
                  </div>
                  <div className="flex justify-between">
                    <span>📐 Phí khoảng cách</span>
                    <span>
                      {dynamicFee.distanceFee.toLocaleString("vi-VN")}₫
                    </span>
                  </div>
                  {dynamicFee.rushHourSurcharge > 0 && (
                    <div className="flex justify-between text-orange-500">
                      <span>
                        ⏰ Phụ thu giờ cao điểm (x
                        {dynamicFee.breakdown.rushHourMultiplier})
                      </span>
                      <span>
                        +{dynamicFee.rushHourSurcharge.toLocaleString("vi-VN")}₫
                      </span>
                    </div>
                  )}
                  {dynamicFee.weatherSurcharge > 0 && (
                    <div className="flex justify-between text-blue-500">
                      <span>
                        🌧️ Phụ thu thời tiết (x
                        {dynamicFee.breakdown.weatherMultiplier})
                      </span>
                      <span>
                        +{dynamicFee.weatherSurcharge.toLocaleString("vi-VN")}₫
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between font-bold text-[#ff6b35] pt-1 border-t">
                    <span>Tổng phí ship</span>
                    <span>{dynamicFee.totalFee.toLocaleString("vi-VN")}₫</span>
                  </div>
                </div>
              )}
              ...
              <div className="mt-2 p-3 bg-gray-100 rounded-xl text-xs text-gray-500 font-mono space-y-0.5">
                <p>
                  ...
                  >
                    {String(profileLoading)}
                  </span>
                </p>
                <p>
                  consumerId:{" "}
                  <span
                    className={consumerId ? "text-green-500" : "text-red-500"}
                  >
                ...
                <p>
                  merchantId:{" "}
                  <span
                    className={merchantId ? "text-green-500" : "text-red-500"}
                  >
                    {merchantId ? "✅ " + merchantId.slice(0, 8) : "❌ null"}
                  </span>
                </p>
                <p>
                  user:{" "}
                ...
                <p>
                  ...
                  {loading || profileLoading ? (
                    <span className="text-red-500 font-bold">
                      🔴 DISABLED (loading={String(loading)}, profileLoading=
                      {String(profileLoading)})
                    </span>
                  ) : (
      ...
      <PromoCodeDrawer
        ...
        appliedCode={promoApplied ? promoCode : null}
        onApply={(promo) => applyPromo(promo)}
      />
    </div>
  );
}
