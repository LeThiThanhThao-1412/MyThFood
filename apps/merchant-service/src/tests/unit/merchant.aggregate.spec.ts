import { Merchant, MerchantStatus, CapacityStatus } from '../../modules/merchant/domain/merchant.aggregate';
import { MerchantId } from '../../modules/merchant/domain/merchant-id';
import { MenuItemId } from '../../modules/merchant/domain/menu-item-id';
import { MenuItem, MenuItemCategory } from '../../modules/merchant/domain/menu-item.entity';
...
describe('Merchant Aggregate', () => {
  ...
  describe('status transitions', () => {
    ...
    it('should not reject an approved merchant', () => {
      merchant.approve();
      expect(() => merchant.reject()).toThrow('Cannot reject an approved merchant');
    });
    ...
    it('should not suspend a rejected merchant', () => {
      merchant.reject();
      expect(() => merchant.suspend()).toThrow('Cannot suspend a rejected merchant');
    });
    ...
    it('should not reactivate a non-suspended merchant', () => {
      expect(() => merchant.reactivate()).toThrow('Only suspended merchants can be reactivated');
    });
  });
  ...
  describe('updateInfo', () => {
    ...
    it('should update logo and cover image URLs', () => {
      merchant.updateInfo({ logoUrl: 'https://img.com/logo.png', coverImageUrl: 'https://img.com/cover.png' });
    ...
    it('should throw when name is set to empty string', () => {
      expect(() => merchant.updateInfo({ name: '' })).toThrow('Merchant name cannot be empty');
    });

    it('should throw when phone is set to empty string', () => {
      expect(() => merchant.updateInfo({ phone: '' })).toThrow('Phone cannot be empty');
    });

    it('should throw when address is set to empty string', () => {
      expect(() => merchant.updateInfo({ address: '' })).toThrow('Address cannot be empty');
    });
  });
  ...
  describe('menu management', () => {
    ...
    it('should emit MenuUpdatedEvent with CREATED action', () => {
      ...
      const events = merchant.pullDomainEvents();
      const menuEvents = events.filter((e) => e.eventType === MENU_UPDATED_EVENT_TYPE);
    ...
    it('should update a menu item name', () => {
      ...
      const updated = merchant.updateMenuItem(menuItem.id, { name: 'Pho Bo Dac Biet' });
    ...
    it('should emit price change event when price is updated', () => {
      ...
      const events = merchant.pullDomainEvents();
      const priceEvents = events.filter((e) => e.eventType === MENU_UPDATED_EVENT_TYPE);
    ...
    it('should throw when adding menu item with empty name', () => {
      expect(() =>
        merchant.addMenuItem({ category: 'MAIN_COURSE', name: '', price: 10000 }),
      ).toThrow('Menu item name is required');
    });

    it('should throw when adding menu item with negative price', () => {
      expect(() =>
        merchant.addMenuItem({ category: 'MAIN_COURSE', name: 'Test', price: -100 }),
  ...
  describe('operating hours', () => {
    ...
    it('should throw when operating hours is empty', () => {
      expect(() => merchant.setOperatingHours([])).toThrow('Operating hours cannot be empty');
    });
    ...
    it('should support overnight hours', () => {
      ...
      merchant.setOperatingHours([
        { dayOfWeek: 1, openTime: '22:00:00', closeTime: '04:00:00', isClosed: false },
      ]);
  ...
  describe('capacity management', () => {
    ...
    it('should throw for invalid max concurrent orders', () => {
      expect(() => merchant.updateCapacityConfig({ maxConcurrentOrders: 0 }))
        .toThrow('Max concurrent orders must be at least 1');
    });

    it('should throw for invalid prep time', () => {
      expect(() => merchant.updateCapacityConfig({ prepTimePerOrder: 0 }))
