...
export class InventoryService {
  ...
  async findByMenuItemId(menuItemId: string): Promise<Inventory> {
    ...
    if (!inv)
      throw new Error(`Inventory for menu item ${menuItemId} not found`);
    return inv;
  }
  ...
  async updateTotal(id: string, dto: UpdateTotalDto): Promise<Inventory> {
    const inv = await this.inventoryRepository.findByIdOrFail(
      InventoryId.from(id),
    );
    ...
    try {
      ...
      for (const { inventory, expiredOrderIds } of expired) {
        for (const orderId of expiredOrderIds) {
          ...
          releasedCount++;
          this.logger.log(
            `Released reservation for order ${orderId} on inventory ${inventory.id}`,
          );
        }
