...
export class ConsumerMapper {
  public static toDomain(entity: ConsumerEntity): Consumer {
    ...
    const favoriteMenuItemIds = ConsumerMapper.parseFavoriteMenuItemIds(
      entity.favorite_menu_item_ids,
    );
  ...
  public static toPersistence(consumer: Consumer): ConsumerEntity {
    ...
    entity.favorite_menu_item_ids = JSON.stringify(
      consumer.favoriteMenuItemIdList,
    );
    return entity;
  }
  ...
  private static parseFavoriteMenuItemIds(json: string): string[] {
    try {
      const raw: unknown = JSON.parse(json ?? "[]");
      if (!Array.isArray(raw)) return [];
      return raw
  ...
  private static parseAddresses(json: string): Address[] {
    try {
      ...
      return raw.map((item: Record<string, unknown>) =>
        ConsumerMapper.deserializeAddress(item),
      );
    } catch {
      return [];
    }
  ...
  private static deserializeAddress(raw: Record<string, unknown>): Address {
    ...
    const gps = gpsRaw
      ? GpsCoordinates.create(
          gpsRaw.latitude as number,
          gpsRaw.longitude as number,
        ).value
      : null;
  ...
  private static serializeAddress(address: Address): Record<string, unknown> {
    return {
      ...
      gps: address.gps
        ? { latitude: address.gps.latitude, longitude: address.gps.longitude }
        : null,
      type: address.type,
  ...
  private static parsePaymentMethods(json: string): PaymentMethod[] {
    try {
      ...
      if (!Array.isArray(raw)) return [];
      return raw.map((item: Record<string, unknown>) =>
        ConsumerMapper.deserializePaymentMethod(item),
      );
    } catch {
      return [];
    }
  ...
  private static deserializePaymentMethod(
  ...
  private static serializePaymentMethod(
